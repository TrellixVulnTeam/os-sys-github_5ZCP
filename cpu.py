...
class CPU(LIBC):
    ...
    def query(cls, cpuids=None, states=None,strictChecking=False):
        ...
        if cpuids and strictChecking and len(cpus) != len(cpuids):
            askedfor = set(cpuids)
            found = set([x.cpuid for x in list(cpus.values())])
...
def get_chips(option, opt_str, value, parser):
    ...
    setattr(parser.values,option.dest,
            sorted([x[0] for x in list(CPU.chips(states=ACTIVE_CPUS).values())]))
...
def get_cores(option, opt_str, value, parser):
    ...
    '''
    setattr(parser.values,option.dest,
            sorted([x[0] for x in list(CPU.cores(states=ACTIVE_CPUS).values())]))
...
if __name__ == '__main__':
    ...
    if options.cpus is None:
        options.cpus = sorted([x[0] for x in list(CPU.chips(states=ACTIVE_CPUS).values())])
